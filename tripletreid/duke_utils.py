...
# img = reader.getFrame(camera, frame)
    ...
    def __init__(self, dataset_path):
        ...
        self.PrevFrame = -1
        self.PrevPart = 0
        self.Video = cv2.VideoCapture('{:s}/dukevideos/camera{:d}/{:05d}.MTS'.format(self.DatasetPath, self.CurrentCamera, self.CurrentPart), cv2.CAP_FFMPEG)
    ...
    def getFrame(self, iCam, iFrame):
        ...
        # Cam 4 77311
        #Compute current frame and in which video part the frame belongs
        ksum = 0
        for k in range(10):
            ksumprev = ksum
